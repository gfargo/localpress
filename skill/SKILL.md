...
**Background removal models.** The `remove-bg` command downloads an AI model on first use. Available models: `birefnet-lite` (~224MB, MIT, best edges, slowest), `isnet-general-use` (~176MB, default, best all-rounder), `u2net` (~176MB, general purpose), `silueta` (~44MB, fast), `u2netp` (~4.7MB, fastest). Note: `u2net` and `silueta` can discard low-contrast clothing on portraits (cutting the subject off at the neck) — prefer the default or `birefnet-lite` for people. Use `--list-models` to check what's cached. Pass `--preview` to open a browser UI for adjusting model, alpha threshold, and background color before applying. Alternatively, use `--rembg` to shell out to system Python rembg if installed.
